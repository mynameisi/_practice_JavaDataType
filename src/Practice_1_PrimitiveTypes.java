/**
 * 这一个类练习的是所有的基本数据类型变量的声明和基本的使用方法
 * 
 * 请按照要求声明和赋值所有的变量
 * 然后右键点击：Practice_1_PrimitiveTypesTests
 * 选择运行方式，JUnit测试，如果没有，双击进入那个文件，然后直接点击运行
 * 目标是所有的变量名对应的测试都是绿色对钩，如果出现红色或者蓝色的× 表示有问题
 * 注意：
 * 以下的最大最小值们，不能用Integer.MAX_VALUE之类的来获得，要求大家能够背诵最大最小值的
 * 
...
public class Practice_1_PrimitiveTypes {
	...
	// 定一个大小为-3.14的double类型变量 x12
	
	// 定一个大小为3.14的double类型变量 x13
	
	// 定一个字符类型变量 x14，使其内容为5
	
	// 定一个布尔类型变量 x14，使其内容为假
